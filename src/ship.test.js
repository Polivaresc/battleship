...
test('create new ship from first position and len', () => {
    const newShip = ship('A1', 3)
    expect(newShip.len).toBe(3)
    expect(newShip.initial_pos).toBe('A1')
    expect(newShip.final_pos === 'A3' || newShip.final_pos === 'C1').toBe(true)
})

test('hit a ship and assert position hit', () => {
    const newShip = ship('B2', 4)
    newShip.hit('B3')
    if(newShip.allPositions.includes('B3')) {
...
test('cannot re-hit position', () => {
    const newShip = ship('A2', 3)
    if(newShip.allPositions.includes('B2')) {
        newShip.hit('B2')
...
test('hit all positions of ship and assert isSunk is true', () => {
    const newShip = ship('A1', 3)
    if(newShip.allPositions === ['A1', 'A2', 'A3']) {
        newShip.hit('A3')
        newShip.hit('A1')
        newShip.hit('A2')
